...
interface DeclineReq {
  userId: string
  friendshipId: string
}

type DeclineRes = Either<ResourceNotFoundError | NotAllowedError, null>

export class DeclineUseCase {
  constructor(
    private friendshipRepository: FriendshipRepository,
    private userRepository: UserRepository
  ) {}

  async execute({ friendshipId, userId }: DeclineReq): Promise<DeclineRes> {
