...
| 05 | [05-websocket.md](./05-websocket.md) | Handshake (cookie + `deviceId` + checagem de `Origin`), códigos de fechamento, de onde vem o cursor de reconexão, heartbeat/presença, onde o backpressure vira palpável no pipeline até o socket, e o limite de escala (1 conexão Redis por device) |
